...
`
  ...
  private generateRoofingRecommendations(data: PropertyData): string {
    ...
    if (propertyDetails.yearBuilt) {
      ...
      if (roofAge > 20) {
        recommendations +=
          "- **Consider Full Replacement**: Based on the property age, your roof may be approaching the end of its serviceable life.\n"
      } else if (roofAge > 15) {
        recommendations +=
          "- **Inspection Recommended**: Your roof is reaching the age where problems typically develop. A thorough inspection is advised.\n"
      } else if (roofAge > 10) {
        recommendations +=
          "- **Preventative Maintenance**: At this age, preventative maintenance can extend roof life and prevent costly repairs.\n"
      } else {
        recommendations +=
          "- **Regular Maintenance**: Your roof is relatively new. Regular maintenance will help protect your investment.\n"
      }
    }

    // Pitch-based recommendations
    const pitch =
      typeof roofDetails.roofPitch === "string"
        ? parseInt(roofDetails.roofPitch.split("/")[0])
        : roofDetails.roofPitch

    if (pitch >= 8) {
      recommendations +=
        "- **Steep Roof Considerations**: Your roof has a steep pitch which affects material choice and installation complexity.\n"
    } else if (pitch <= 3) {
      recommendations +=
        "- **Low-Slope Solutions**: Your roof has a relatively low pitch which requires special materials to prevent water pooling.\n"
    }

    // Material recommendations
    recommendations += "- **Material Options**:\n"
    recommendations +=
      "  - **Asphalt Shingles**: Economical and widely used, lifespan of 15-30 years depending on quality\n"
    recommendations +=
      "  - **Metal Roofing**: Longer lifespan (40-70 years), energy efficient, higher upfront cost\n"
    recommendations +=
      "  - **Clay Tile**: Aesthetically pleasing, excellent durability, higher cost and weight\n"
