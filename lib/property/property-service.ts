...
export class PropertyResearchService {
  ...
  async researchProperty(address: DetectedAddress): Promise<PropertyData> {
    try {
      console.log(
        `[INFO] Starting research for property at: ${address.fullAddress}`
      )
  ...
  private async geocodeAddress(
    ...
    try {
      ...
      console.log(
        `Making direct geocoding request to Google API for: ${address}`
      )
      ...
      if (data.status !== "OK" || !data.results || data.results.length === 0) {
        throw new Error(
          `Geocoding failed with status: ${data.status}, error: ${data.error_message || "No results found"}`
        )
      }
  ...
  private async fetchSolarData(lat: number, lng: number): Promise<any> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Solar API HTTP error: ${response.status} ${response.statusText}, Details: ${errorText}`
        )
      }
      ...
      if (!data.solarPotential) {
        throw new Error(
          "Invalid Solar API response: missing solarPotential data"
        )
      }
  ...
  private extractPropertyDetails(solarData: any): PropertyDetails {
    try {
      ...
      // Log available data
      console.log(
        `[DEBUG] Available wholeRoofStats keys: ${Object.keys(wholeRoofStats).join(", ")}`
      )
  ...
  private extractRoofDetails(solarData: any): RoofDetails {
    try {
      ...
      const facets = roofSegmentStats.map((segment: any, index: number) => {
        ...
        const areaSqFt = stats.areaMeters2
          ? Math.round(stats.areaMeters2 * conversionFactor * 100) / 100
          : 0

        // Calculate percentage of total roof area
        const percentageOfRoof =
          roofAreaSqFt > 0 ? (areaSqFt / roofAreaSqFt) * 100 : 0
        ...
        return {
          ...
          orientation: azimuthDegrees
            ? this.degreesToOrientation(azimuthDegrees)
            : "Unknown",
      ...
      const maxPitchDegrees =
        roofSegmentStats.length > 0
          ? Math.max(
              ...roofSegmentStats.map(
                (segment: any) => segment.pitchDegrees || 0
              )
            )
          : 0
  ...
  private getMostCommonPitch(facets: RoofFacet[]): string {
    ...
    const pitchCounts = facets.reduce(
      (counts, facet) => {
  ...
  private extractSolarPotential(solarData: any): SolarPotential {
    try {
      ...
      // Log available data
      console.log(
        `[DEBUG] Available solarPotential keys: ${Object.keys(solarPotential).join(", ")}`
      )
      console.log(
        `[DEBUG] Number of financial analyses: ${financialAnalyses.length}`
      )
      console.log(
        `[DEBUG] Number of solar panel configs: ${solarPanelConfigs.length}`
      )
      ...
      financialAnalyses.forEach((config: any) => {
        console.log(
          `[DEBUG] Checking financial analysis config, has cashPurchaseSavings: ${!!config.cashPurchaseSavings}`
        )
      ...
      if (bestConfig) {
        ...
        costOfElectricityWithoutSolar = Number(
          financialDetails.costOfElectricityWithoutSolar?.units || 0
        )
        remainingLifetimeUtilityBill = Number(
          financialDetails.remainingLifetimeUtilityBill?.units || 0
        )
        monthlyBill = Number(bestConfig.monthlyBill?.units || 0)

        console.log(`[DEBUG] Installation cost: $${installationCost}`)
        console.log(
          `[DEBUG] Cost without solar: $${costOfElectricityWithoutSolar}`
        )
        console.log(
          `[DEBUG] Remaining utility bill: $${remainingLifetimeUtilityBill}`
        )
      } else {
      ...
      const bestPanelConfig =
        solarPanelConfigs.length > 0 ? solarPanelConfigs[0] : null
      ...
      if (bestConfig && bestConfig.paybackYears) {
        ...
        paybackPeriodYears =
          annualSavings > 0 ? installationCost / annualSavings : 0
      }
      ...
      const yearlyEnergyDcKwh =
        bestPanelConfig?.yearlyEnergyDcKwh ||
