...
export async function GET() {
  ...
  try {
    ...
    if (userError) {
      ...
      results.push(
        `✅ Found existing user for testing: ${realUserId.substring(0, 8)}...`
      )
      ...
      if (insertError) {
        if (insertError.message.includes("duplicate key value")) {
          results.push(
            "✅ Subscription constraint working (user already has subscription)"
          )
        } else {
        ...
        // Clean up test data
        await supabase.from("subscriptions").delete().eq("user_id", realUserId)
      ...
      if (usageInsertError) {
        results.push(
          `❌ Feature usage insert failed: ${usageInsertError.message}`
        )
        return NextResponse.json({ results, success: false })
      }
      ...
      await supabase.from("feature_usage").delete().eq("user_id", realUserId)
      results.push("✅ Can delete from feature_usage table")
    } else {
      results.push(
        "ℹ️ No existing users found - will test with real users later"
      )
    }
    ...
    if (fkError && fkError.message.includes("foreign key constraint")) {
      results.push(
        "✅ Foreign key constraints working correctly (prevents fake users)"
      )
    } else {
