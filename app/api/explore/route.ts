...
export async function POST(request: Request) {
  try {
    ...
    console.log(
      `Processing coordinates: ${lat}, ${lng}, workspace: ${workspaceId || "not provided"}`
    )
    ...
    try {
      const propertyData =
        await propertyService.researchProperty(detectedAddress)
      ...
      if (enhancedAnalysis) {
        console.log(
          "Enhanced analysis data available, incorporating into report"
        )
        ...
        if (structuredData.facetCount) {
          console.log(
            `Updating roof facets from ${enhancedPropertyData.roofFacets || "unknown"} to ${structuredData.facetCount}`
          )
        ...
        if (structuredData.ridgeLength) {
          console.log(
            `Updating ridge length from ${enhancedPropertyData.ridgeLength || "unknown"} to ${structuredData.ridgeLength}`
          )
        ...
        if (structuredData.valleyLength) {
          console.log(
            `Updating valley length from ${enhancedPropertyData.valleyLength || "unknown"} to ${structuredData.valleyLength}`
          )
          enhancedPropertyData.valleyLength = structuredData.valleyLength
        }

        // Add additional analysis data
        enhancedPropertyData.roofComplexity =
          structuredData.complexity || enhancedPropertyData.roofComplexity
      ...
      if (measuredArea) {
        console.log(
          `Incorporating manually measured area: ${measuredArea} sq ft`
        )
...
function extractAddressComponent(addressResult: any, type: string): string {
  try {
    const component = addressResult.address_components.find((comp: any) =>
