...
const AERIS_CLIENT_ID =
  process.env.NEXT_PUBLIC_AERISWEATHER_CLIENT_ID ||
  "YOUR_AERISWEATHER_CLIENT_ID"
const AERIS_CLIENT_SECRET =
  process.env.NEXT_PUBLIC_AERISWEATHER_CLIENT_SECRET ||
  "YOUR_AERISWEATHER_CLIENT_SECRET"
...
export default function RooferWeatherWidget({
  ...
  const detectApiConfigurationIssues = () => {
    if (
      ...
      throw new Error(
        "OpenWeatherMap API key is not configured. Please set it in your environment variables."
      )
    }

    if (
      !AERIS_CLIENT_ID ||
      AERIS_CLIENT_ID === "YOUR_AERISWEATHER_CLIENT_ID" ||
      !AERIS_CLIENT_SECRET ||
      AERIS_CLIENT_SECRET === "YOUR_AERISWEATHER_CLIENT_SECRET"
    ) {
      console.warn(
        "AerisWeather API credentials not configured. Falling back to OpenWeatherMap for hail data."
      )
    }
  }
  ...
  const fetchAerisHailData = async (lat: number, lon: number) => {
    try {
      ...
      console.log(
        `Fetching AerisWeather hail data from ${fromDate} to ${toDate}`
      )
      ...
      if (!response.ok) {
        throw new Error(
          `AerisWeather API error: ${response.status} ${response.statusText}`
        )
      }
      ...
      if (!data.success) {
        throw new Error(
          `AerisWeather API returned error: ${data.error?.description || "Unknown error"}`
        )
      }
      ...
      const hailEvents = data.response
        ...
        .map((report: any) => {
          ...
          const timestamp = report.dateTimeISO
            ? new Date(report.dateTimeISO).getTime() / 1000
            : report.timestamp
              ? report.timestamp
          ...
          if (report.report && report.report.size) {
            ...
            sizeInches =
              report.report.size.inches || report.report.size.value || 0
            sizeName = report.report.size.name || "hail"
          } else if (report.detail && report.detail.hailSize) {
            // Alternative structure
            sizeInches =
              report.detail.hailSize.inches || report.detail.hailSize || 0
            sizeName =
              report.detail.hailSize >= 1
                ? "Large"
                : report.detail.hailSize >= 0.5
                  ? "Medium"
                  : "Small"
      ...
      console.log(
        `Found ${hailEvents.length} hail events from AerisWeather:`,
        hailEvents
      )
  ...
  const fetchHistoricalHailData = async (lat: number, lon: number) => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          "No AerisWeather hail data found, falling back to OpenWeatherMap"
        )
      } catch (aerisError) {
        console.warn(
          "AerisWeather fetch failed, falling back to OpenWeatherMap:",
          aerisError
        )
      }
      ...
      for (let batchIndex = 0; batchIndex < batches; batchIndex++) {
        ...
        const batchRequests = batchTimestamps.map(timestamp =>
          fetch(
            `https://api.openweathermap.org/data/3.0/onecall/timemachine?lat=${lat}&lon=${lon}&dt=${timestamp}&units=imperial&appid=${OPENWEATHER_API_KEY}`
          )
            .then(response => {
              if (!response.ok) {
                // Don't throw, just return null for unsuccessful requests
                console.warn(
                  `Historical data fetch failed for day ${timestamp}: ${response.status}`
                )
                return null
              }
            ...
            .catch(err => {
              console.warn(
                `Error fetching historical data for day ${timestamp}:`,
                err
              )
        ...
        setHailLoadingProgress(
          Math.min(100, Math.round(((batchIndex + 1) / batches) * 100))
        )
      }
  ...
  const detectHailEventsInData = (data: any) => {
    ...
    data.data.forEach((hourData: any) => {
      if (
        ...
        hourData.weather &&
        hourData.weather.some(
          (w: any) =>
            w.id === 906 ||
            w.description.toLowerCase().includes("hail") ||
            (w.id >= 200 &&
              w.id < 300 &&
              w.description.toLowerCase().includes("heavy"))
        )
      ) {
        hailEvents.push({
          time: hourData.dt,
      ...
      try {
        ...
        if (zipCode) {
          ...
          if (!zipResponse.ok) {
            if (zipResponse.status === 401) {
              throw new Error(
                "API key invalid. Please check your OpenWeatherMap API key."
              )
            } else if (zipResponse.status === 404) {
              throw new Error(
                `ZIP code "${zipCode}" not found. Please verify the ZIP code.`
              )
            } else {
              throw new Error(
                `Error looking up ZIP code: ${zipResponse.status} ${zipResponse.statusText}`
              )
            }
          }
          ...
          if (!zipData || !zipData.lat || !zipData.lon) {
            throw new Error(
              `Location data not available for ZIP code "${zipCode}"`
            )
          }
          ...
          if (!geoResponse.ok) {
            if (geoResponse.status === 401) {
              ...
              throw new Error(
                `Error looking up location: ${geoResponse.status} ${geoResponse.statusText}`
              )
            }
          }
          ...
          if (!geoData || geoData.length === 0) {
            throw new Error(
              `Location "${queryLocation}" not found. Please try a different location format.`
            )
          }
          ...
          if (!location.includes("(")) {
            ...
            if (locationData.country && locationData.country !== "US")
        ...
        if (!weatherResponse.ok) {
          if (weatherResponse.status === 401) {
            ...
            throw new Error(
              `Failed to fetch weather data: ${weatherResponse.status} ${weatherResponse.statusText}`
            )
          }
        }
        ...
        try {
          ...
          console.log(
            `Found ${allHailEvents.length} hail events in the last 30 days and upcoming forecast`
          )
        } catch (histErr) {
        ...
        setError(
          err.message || "An unknown error occurred while fetching weather data"
        )
      } finally {
        setIsLoading(false)
      }
  ...
  const isRoofingSafe = (data: any) => {
    ...
    // Check for hail risk
    if (
      data.weather[0].id === 906 ||
      data.weather[0].description.toLowerCase().includes("hail")
    ) {
  ...
  const nextPrecipitation = hourly.find(
    (hour: any) =>
      hour.pop > 0.3 ||
  ...
  return (
    <div className="mx-auto max-w-md overflow-hidden rounded-[5px] bg-white shadow-lg">
      ...
      <div
        className={`p-2 text-center font-bold text-white ${safetyStatus.safe ? "bg-green-600" : "bg-red-600"}`}
      >
        {safetyStatus.safe
              ...
              {nextPrecipitation ? (
                <div className="mb-4">
                  <div className="font-semibold">
                    Next precipitation expected:
                  </div>
                  <div className="flex items-center text-blue-600">
                    <Calendar className="mr-2" size={16} />
                    {formatDate(nextPrecipitation.dt)} at{" "}
                    {formatTime(nextPrecipitation.dt)}
                    <span className="ml-2">
                      ({Math.round(nextPrecipitation.pop * 100)}% chance)
                    </span>
                  </div>
                </div>
              ) : (
            ...
            <div className="mt-3">
              <div className="mb-4 grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div className="text-lg">
                    {Math.round(current.wind_gust || current.wind_speed)} mph
                  </div>
                </div>
                    ...
                    >
                      ...
                      <div className="mx-1 flex flex-1 items-center">
                        <div
                          ...
                          style={{
                            width: `${Math.min(hour.wind_speed * 2, 100)}%`
                          }}
                        ></div>
                      </div>
              ...
              {hailData && hailData.length > 0 ? (
                <div>
                  <div className="mb-4 rounded-[5px] bg-amber-100 p-3 text-amber-800">
                    ...
                    <p className="text-sm">
                      {hailData.length} hail event
                      {hailData.length !== 1 ? "s" : ""} detected in this area
                      within the last 30 days. Consider scheduling a roof
                      inspection.
                    </p>
                  </div>
                  ...
                  <div className="mt-4 rounded bg-gray-50 p-2 text-sm">
                    ...
                    <ul className="mt-1 list-disc space-y-1 pl-4 text-xs">
                      <li>
                        Small hail (under 0.75&quot;): typically won&apos;t
                        damage most roofs
                      </li>
                      <li>
                        Medium hail (0.75&quot;-1.5&quot;): may damage asphalt
                        shingles
                      </li>
                      <li>
                        Large hail (over 1.5&quot;): almost always causes roof
                        damage
                      </li>
                    </ul>
                  </div>
                ...
                <div className="py-4 text-center">
                  <div className="font-medium text-green-600">
                    No recent hail events detected
                  </div>
                  <p className="mt-1 text-sm text-gray-500">
                    No significant hail activity has been reported in this area
                    in the last 30 days.
                  </p>
                </div>
              )}
            ...
            >
              {daily.slice(0, 5).map((day: any, index: number) => {
                ...
                return (
                  <div
                    key={index}
                    className="flex items-center border-b py-2 last:border-0"
                  >
                ...
                {weatherData.alerts.map((alert: any, index: number) => (
                  ...
                  >
                    ...
                    <div className="text-xs">
                      {new Date(alert.start * 1000).toLocaleString()} to{" "}
