...
export const ChatWeatherLookup = ({
  ...
  const extractLocationWithLLM = async (
    input: string
  ): Promise<string | null> => {
...
`
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (location && typeof location === "string" && location.length > 1) {
            ...
            if (
              ["unknown", "not specified", "no location"].includes(
        ...
        console.warn(
          "AI API location extraction failed, falling back to pattern matching:",
          e
        )
      }
  ...
  const extractLocationWithPatterns = (text: string): string | null => {
    ...
    const htmlCommentMatch = text.match(
  ...
  useEffect(() => {
    const processLocation = async () => {
      ...
      const explicitLocationInMessage =
        extractLocationWithPatterns(messageContent)
      if (explicitLocationInMessage) {
        console.log(
          "Found explicit weather location in message:",
          explicitLocationInMessage
        )
