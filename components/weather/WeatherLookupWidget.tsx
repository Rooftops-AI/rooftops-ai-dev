...
export default function WeatherLookupWidget({
  ...
  const handleSelectSuggestion = (suggestion: any) => {
    ...
    if (
      ...
      if (searchTerm) {
        setAddress(
          searchTerm +
            (suggestion.extractedState ? `, ${suggestion.extractedState}` : "")
        )
        setTimeout(() => {
          handleSubmit(new Event("submit") as React.FormEvent)
        }, 100)
        ...
        setError(
          "Unable to extract city from address. Please enter a city name directly."
        )
      }
      return
    }
  ...
  const handleLocationSelect = async (locationData: any) => {
    ...
    try {
      ...
      if (locationData.zip) {
        ...
        const countryPart = locationData.country
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          throw new Error(
            `Unable to validate ZIP code: ${response.statusText || "API Error"} (${response.status})`
          )
        }
      }
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          throw new Error(
            `Unable to validate location: ${response.statusText || "API Error"} (${response.status})`
          )
        }
      }
      ...
      if (!data || (Array.isArray(data) && data.length === 0)) {
        throw new Error(
          `Location "${query}" not found. Please check your input and try again.`
        )
      }
  ...
  const geocodeQuery = async (
    ...
    if (zipCode && isZipCodeOnly(trimmedQuery) && !forceTextSearch) {
      try {
        ...
        throw new Error(
          error.message || "ZIP code not found. Please check and try again."
        )
      }
    }
    ...
    if (zipCode && !isZipCodeOnly(trimmedQuery) && !forceTextSearch) {
      try {
        ...
        console.warn(
          "ZIP code geocoding failed, will try alternative methods:",
          error
        )
    ...
    if (hasHouseNumber(trimmedQuery)) {
      ...
      const cityMatch = trimmedQuery.match(
      ...
      try {
        ...
        throw new Error(
          "Address not found. Try entering a city name or ZIP code instead."
        )
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent, forceTextSearch = false) => {
    ...
    try {
      ...
      setError(
        err.message ||
          "Failed to find location. Try a different format or name."
      )
    } finally {
      setIsValidating(false)
    }
  ...
  const detectUserLocation = async () => {
    ...
    try {
      ...
      if (error.code === 1) {
        // PERMISSION_DENIED
        setError(
          "Location access denied. Please allow location access or enter location manually."
        )
      } else {
        setError(
  ...
  const shouldShowDropdown =
    showDropdown && (suggestions.length > 0 || recentLocations.length > 0)
  ...
  return (
    <div className="pt-4">
      {!showWeather ? (
        ...
        <RooferWeatherWidget location={location} onClose={handleClose} />
      )}
    </div>
  )
}
